function MEA_GUI_WithWorkingH5()
...
%% ============================================================================
        ...
        % Step 3: Ask for noisy channels and replace with zeros
        channelNames = fieldnames(channelData);
        
        % Get previous noisy channels from session settings
        sessionSettings = getappdata(fig, 'sessionSettings');
        ...
        % Create dialog for noisy channels with previous value as default
        prompt = {'Enter noisy channels separated by commas (e.g., A2,B3,C4):'};
        dlgtitle = 'Noisy Channels';
        dims = [1 60];
        definput = {previousNoisyChannels};  % Pre-populate with previous value
        answer = inputdlg(prompt, dlgtitle, dims, definput);
        
        if ~isempty(answer)
            noisyChannelsStr = answer{1};
            
            % Save to session settings (even if empty - user might want to clear it)
            ...
            for i = 1:length(channelList)
                ...
                if ismember(channeldel, channelNames)
                    % Replace with zeros in BOTH structures
            ...
            % Step 4: Re-apply high-pass filter to ALL channels
            ...
            end  % end if ~isempty(noisyChannelsStr)
        end  % end if ~isempty(answer)
...
function createEventSpikeSummary(spikeDataFolder, eventSpikes)
    ...
    addStatus('Cell-Type Classification complete.');
        ...
        % Launch the Channel Inspector tool in a new figure
        addStatus('Launching Channel Inspector...');
        try
            channelInspectorTool();
            addStatus('Channel Inspector opened');
        catch ME
            addStatus(['ERROR launching Channel Inspector: ' ME.message]);
        end
    end
...
masterDBPath = fullfile(parentDir, 'MEA_Master_Database.xlsx');
...
function saveLayerMapFigure()
    ...
    % NOTE: This must be a nested function inside MEA_GUI_WithWorkingH5
...
end  % ← END der Hauptfunktion MEA_GUI_WithWorkingH5
